...
must_haves:
  truths:
    - "constants.ts exports all UUIDs needed by every other file in the app with zero duplicates"
    - "application.config.ts exports a valid defineApplication() with metadata, icon, and linked role"
    - "default.role.ts exports a valid defineRole() with canReadAllObjectRecords: true"
    - "pre-install and post-install logic functions export valid definePreInstallLogicFunction/definePostInstallLogicFunction calls"
  artifacts:
    - path: "packages/twenty-consultancy/src/constants.ts"
      provides: "All UUIDs for fields, views, nav items, filters, options, and scaffold entities"
    - path: "packages/twenty-consultancy/application.config.ts"
      provides: "App entry point with defineApplication()"
    - path: "packages/twenty-consultancy/src/roles/default.role.ts"
      provides: "Default role with read-all permissions"
    - path: "packages/twenty-consultancy/src/logic-functions/pre-install.logic-function.ts"
      provides: "Pre-install validation hook"
    - path: "packages/twenty-consultancy/src/logic-functions/post-install.logic-function.ts"
      provides: "Post-install success hook"
    - path: "packages/twenty-consultancy/package.json"
      provides: "Package manifest with twenty-sdk dependency"
    - path: "packages/twenty-consultancy/tsconfig.json"
      provides: "TypeScript configuration"
  key_links:
    - from: "packages/twenty-consultancy/application.config.ts"
      to: "packages/twenty-consultancy/src/roles/default.role.ts"
      via: "defaultRoleUniversalIdentifier import"
...
<objective>
Create the HRMS Hub app scaffold: package configuration, constants.ts with ALL UUIDs, defineApplication config, default role, and pre/post install logic functions.

Purpose: Establishes the foundational app structure and UUID registry that all subsequent plans depend on. Every field, view, and nav item file will import UUIDs from constants.ts created here.

Output: 7 files forming a complete, valid Twenty SDK app scaffold ready for entity files.
</objective>
...
<task type="auto">
  <name>Task 1: Create package config, tsconfig, and constants.ts with all UUIDs</name>
  <files>
    packages/twenty-consultancy/package.json
    packages/twenty-consultancy/tsconfig.json
    packages/twenty-consultancy/src/constants.ts
  ...
  <action>
Create three files. The existing files in packages/twenty-consultancy/ (README.md, contracts/, immigration/, compliance/, scripts/) are old metadata scripts -- leave them in place. We are adding new SDK app files alongside them.

**1. packages/twenty-consultancy/package.json**

Write this exact content:

```json
{
  "name": "twenty-consultancy",
  "version": "0.1.0",
  "license": "MIT",
  "engines": {
    "node": "^24.5.0",
...
**2. packages/twenty-consultancy/tsconfig.json**
...
**3. packages/twenty-consultancy/src/constants.ts**

Write this exact content. These UUIDs are final and must never change once committed:
...
IMPORTANT: Do not modify or remove any existing files in packages/twenty-consultancy/. Only add new files.
  </action>
  <verify>
Run: `cat packages/twenty-consultancy/src/constants.ts | head -5` to confirm file exists.
Run: `cat packages/twenty-consultancy/package.json | head -3` to confirm package.json exists.
Run: `cat packages/twenty-consultancy/tsconfig.json | head -3` to confirm tsconfig.json exists.
Grep for duplicate UUIDs: `grep -oP "'[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}'" packages/twenty-consultancy/src/constants.ts | sort | uniq -d` should return empty (no duplicates).
  </verify>
  <done>
constants.ts contains all UUIDs for the entire Phase 1 app (scaffold, 9 fields, 6 views, 6 nav items, filters, view fields, SELECT options). package.json and tsconfig.json are valid. No duplicate UUIDs exist.
  </done>
</task>

<task type="auto">
  <name>Task 2: Create application config, default role, and logic functions</name>
  <files>
    packages/twenty-consultancy/application.config.ts
    packages/twenty-consultancy/src/roles/default.role.ts
    packages/twenty-consultancy/src/logic-functions/pre-install.logic-function.ts
  ...
  <action>
Create four files. Each must use `export default` with the appropriate define function.

**1. packages/twenty-consultancy/application.config.ts**
...
**2. packages/twenty-consultancy/src/roles/default.role.ts**
...
**3. packages/twenty-consultancy/src/logic-functions/pre-install.logic-function.ts**
...
**4. packages/twenty-consultancy/src/logic-functions/post-install.logic-function.ts**
...
Ensure parent directories exist before writing: `src/roles/` and `src/logic-functions/`.
  </action>
  <verify>
Verify each file has `export default`:
- `grep 'export default' packages/twenty-consultancy/application.config.ts`
- `grep 'export default' packages/twenty-consultancy/src/roles/default.role.ts`
- `grep 'export default' packages/twenty-consultancy/src/logic-functions/pre-install.logic-function.ts`
- `grep 'export default' packages/twenty-consultancy/src/logic-functions/post-install.logic-function.ts`

Verify imports reference constants.ts:
- `grep "from.*constants" packages/twenty-consultancy/application.config.ts`
- `grep "from.*constants" packages/twenty-consultancy/src/roles/default.role.ts`
  </verify>
  <done>
application.config.ts defines the HRMS Hub app with icon, description, and linked role UUID. default.role.ts grants canReadAllObjectRecords. Pre/post install functions log installation state. All UUIDs imported from constants.ts — no inline UUIDs.
  </done>
</task>
...
<verification>
1. All 7 files exist under packages/twenty-consultancy/
2. constants.ts has zero duplicate UUIDs
3. Every file that uses a UUID imports it from constants.ts (no hardcoded UUIDs except standard Person object/field IDs in constants.ts itself)
4. Every entity file uses `export default defineXxx(...)` pattern
5. application.config.ts references the role UUID from constants
6. package.json has twenty-sdk as dependency
</verification>

<success_criteria>
- 7 files created without errors
- App scaffold is structurally valid (one export default per entity file)
- All UUIDs centralized in constants.ts
- No duplicate UUIDs across the entire constants file
- Role grants canReadAllObjectRecords: true
- Logic functions have valid handler signatures
</success_criteria>

<output>
After completion, create `.planning/phases/01-person-centric-foundation/01-01-SUMMARY.md`
</output>
